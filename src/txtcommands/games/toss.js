...
export async function toss(id, amount, channel, choice = "head") {
  try {
    ...
    if (random === 1 && choice === "head") {
      await suspenseMessage.edit(`🎉 **${guild.user.username}**, you did it! 🪙\nThe coin landed on heads! You won <:kasiko_coin:1300141236841086977>**${Number(2* winamount).toLocaleString()}** 𝑪𝒂𝒔𝒉! Fortune is on your side today!`);
    } else if (random === 0 && choice === "tail") {
      await suspenseMessage.edit(`🎉 **${guild.user.username}**, victory is yours! 🪙\nThe coin landed on tails! You won <:kasiko_coin:1300141236841086977>**${Number(2* winamount).toLocaleString()}** 𝑪𝒂𝒔𝒉! Luck favors you this time!`);
    } else {

...
import { generateStockChart } from "./canvas.js";

import { buySharesCommand } from "./req/buy.js";

import { portfolioCommand } from "./req/portfolio.js";

import { sellSharesCommand } from "./req/sell.js";
...
export function createStockEmbed(name, stock, ownerView = false) {
  ...
  const generalInfoEmbed = new EmbedBuilder()
    ...
    .addFields(
      ...
      },
      {
        name: "𝘾𝙪𝙧𝙧𝙚𝙣𝙩 𝙎𝙩𝙖𝙩𝙪𝙨",
        value:
          `**Current Price**: <:kasiko_coin:1300141236841086977> **${stock.currentPrice.toLocaleString()}**\n` +
          `**Trend**: ${capitalizeFirstLetter(stock.trend)} ${
            stock.trend.toLowerCase() === "up"
              ? "<:stocks_profit:1321342107574599691>"
              : stock.trend.toLowerCase() === "down"
              ? "<:stocks_loss:1321342088020885525>"
              : "~"
          }\n` +
  ...
  if (stock.image) {
    generalInfoEmbed.setThumbnail(
      `https://cdn.discordapp.com/app-assets/${APPTOKEN}/${stock.image}.png`
    );
  }
    ...
    .addFields(
      {
        ...
        value:
          `**P/E Ratio**: ${stock.PEratio ? stock.PEratio : stock.peRatio}\n` +
      ...
      },
      {
        ...
        value:
          `**High**: <:kasiko_coin:1300141236841086977> ${Math.max(
            ...stock.last10Prices
          ).toLocaleString()}\n` +
          `**Low**: <:kasiko_coin:1300141236841086977> ${Math.min(
            ...stock.last10Prices
          ).toLocaleString()}`,
        inline: false,
      }
  ...
  if (ownerView) {
    const shareholdersCount = stock.shareholders
      ? stock.shareholders.length
      : 0;

    const ownerDetails =
      `**Authorized Shares:** ${stock.authorizedShares}\n` +
      ...
      `**Last Work At:** ${
        stock.lastWorkAt ? new Date(stock.lastWorkAt).toLocaleString() : "N/A"
      }\n` +
      `**Last Salary Withdrawal:** ${
        stock.lastSalaryWithdrawal
          ? new Date(stock.lastSalaryWithdrawal).toLocaleString()
          : "N/A"
      }\n` +
      `**IPO Date:** ${
        stock.ipoDate ? new Date(stock.ipoDate).toLocaleString() : "N/A"
      }\n` +
      `**Funding Rounds:** ${
        stock.fundingRounds ? stock.fundingRounds.length : 0
      }`;
    ...
    return [generalInfoEmbed, financialDetailsEmbed, ownerViewEmbed];
  }
...
export async function sendPaginatedStocks(context) {
  try {
    ...
    const initialEmbed = createStockEmbed(
    ...
    collector.on("collect", async (interaction) => {
      ...
      try {
        ...
        const newEmbed = createStockEmbed(
          companies[currentIndex].name,
          companies[currentIndex]
        );
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${
          company.name
        }** is currently priced at <:kasiko_coin:1300141236841086977> **${company.currentPrice.toLocaleString()}** Cash.`
      )
    ...
    return {
      content:
        "⚠️ Something went wrong while checking the company's stock price.",
    };
  }
}
        ...
        default: {
          const embed = new EmbedBuilder()
            ...
            .addFields(
              {
                name: "`stock all`",
                value: "View all available stocks",
                inline: true,
              },
              {
                name: "`stock price <symbol>`",
                value: "Check the price of a stock",
                inline: true,
              },
              {
                name: "`stock buy <symbol> <amount>`",
                value: "Buy a stock",
                inline: true,
              },
              {
                name: "`stock sell <symbol> <amount>`",
                value: "Sell a stock",
                inline: true,
              },
              {
                name: "`stock news`",
                value: "Get the latest stock news",
                inline: true,
              },
              {
                name: "`stock portfolio`",
                value: "View your stock portfolio",
                inline: true,
              }
            );
          return message.channel
